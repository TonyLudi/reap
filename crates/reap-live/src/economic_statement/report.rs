...
use super::{
    BoundEconomicSources, DerivativePnlFormulaSample, ECONOMIC_RECONCILIATION_SCHEMA_VERSION,
    EconomicIssueSource, EconomicJournalRecoveryEvidence, EconomicReconciliationCounts,
    EconomicReconciliationFailure, EconomicReconciliationOptions, EconomicReconciliationReport,
    EconomicReconciliationScope, FundingFormulaSample, IssueSink, JournalFundingSettlement,
    JournalMarkPriceObservation, JournalRuntimeSession, JournalTradeEvidence,
    MAX_ECONOMIC_DERIVATIVE_PNL_SAMPLES, MAX_ECONOMIC_FUNDING_SAMPLES,
};

type FillKey = (String, String);
type FillByKey<'a> = BTreeMap<FillKey, &'a RemoteFill>;
type JournalTradeEvidenceByKey = BTreeMap<FillKey, Vec<JournalTradeEvidence>>;

struct BillValidationState {
    matched_fill_keys: BTreeSet<FillKey>,
    derivative_pnl_samples: Vec<DerivativePnlFormulaSample>,
    derivative_pnl_samples_omitted: u64,
    funding_samples: Vec<FundingFormulaSample>,
    funding_samples_omitted: u64,
}
...
pub(super) fn build_report(
    ...
    let completeness_end = classify_fill_windows(&sources, &options, &mut counts);
    let fill_by_key = index_verified_fills(&sources.fills, &mut failures, &mut issues);
    ...
    let bill_validation = validate_bill_collection(
        &sources,
        &options,
        &fill_by_key,
    ...
    validate_fill_bill_completeness(
        &sources.fills,
        &options,
        completeness_end,
        &bill_validation.matched_fill_keys,
        &mut counts,
        &mut failures,
        &mut issues,
    );
    record_minimum_failures(&counts, &options, &mut failures);
    ...
    EconomicReconciliationReport {
        ...
        counts,
        derivative_pnl_formula_samples: bill_validation.derivative_pnl_samples,
        derivative_pnl_formula_samples_omitted: bill_validation
            .derivative_pnl_samples_omitted,
        funding_formula_samples: bill_validation.funding_samples,
        funding_formula_samples_omitted: bill_validation.funding_samples_omitted,
...
fn validate_bill_collection(
    sources: &BoundEconomicSources,
    options: &EconomicReconciliationOptions,
    fill_by_key: &FillByKey<'_>,
    journal_trade_evidence: &JournalTradeEvidenceByKey,
    valid_settlements: &[&JournalFundingSettlement],
    valid_runtime_sessions: &[&JournalRuntimeSession],
    valid_mark_prices: &[&JournalMarkPriceObservation],
    counts: &mut EconomicReconciliationCounts,
    failures: &mut BTreeSet<EconomicReconciliationFailure>,
    issues: &mut IssueSink,
) -> BillValidationState {
    let mut trade_bill_keys = BTreeSet::new();
    let mut state = BillValidationState {
        matched_fill_keys: BTreeSet::new(),
        derivative_pnl_samples: Vec::new(),
        derivative_pnl_samples_omitted: 0,
        funding_samples: Vec::new(),
        funding_samples_omitted: 0,
    };
    ...
    for bill in &sources.bills {
        match bill.bill_type.as_str() {
            "2" => {
                ...
                state.matched_fill_keys.insert(key.clone());
                ...
                if let Some(sample) = validation.derivative_sample {
                    ...
                    if state.derivative_pnl_samples.len() < MAX_ECONOMIC_DERIVATIVE_PNL_SAMPLES {
                        state.derivative_pnl_samples.push(sample);
                    } else {
                        state.derivative_pnl_samples_omitted =
                            state.derivative_pnl_samples_omitted.saturating_add(1);
                    }
                }
            }
            "8" => {
                ...
                if let Some(sample) = sample {
                    if state.funding_samples.len() < MAX_ECONOMIC_FUNDING_SAMPLES {
                        state.funding_samples.push(sample);
                    } else {
                        state.funding_samples_omitted =
                            state.funding_samples_omitted.saturating_add(1);
                    }
                }
            }
            _ => {
...
#[allow(clippy::too_many_arguments)]
fn validate_fill_bill_completeness(
    fills: &[RemoteFill],
    options: &EconomicReconciliationOptions,
    completeness_end: u64,
    matched_fill_keys: &BTreeSet<FillKey>,
    counts: &mut EconomicReconciliationCounts,
    failures: &mut BTreeSet<EconomicReconciliationFailure>,
    issues: &mut IssueSink,
...
fn record_minimum_failures(
    counts: &EconomicReconciliationCounts,
    options: &EconomicReconciliationOptions,
    failures: &mut BTreeSet<EconomicReconciliationFailure>,
) {
