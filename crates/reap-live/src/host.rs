...
pub(crate) struct HostGuardStats {
    pub checks: u64,
    pub last_snapshot: Option<HostHealthSnapshot>,
}

pub(crate) struct HostGuardRuntime {
...
pub(crate) fn check_host_health(
    config: &HostGuardConfig,
    journal_path: &Path,
) -> Result<HostHealthSnapshot, HostHealthError> {
    let snapshot = probe_host(journal_path)?;
    evaluate_host_health(config, snapshot)
}

pub(crate) fn start_host_guard(config: HostGuardConfig, journal_path: PathBuf) -> HostGuardRuntime {
        ...
        loop {
            tokio::select! {
                _ = tokio::time::sleep(interval) => {
                    ...
                    let check_path = journal_path.clone();
...
fn evaluate_host_health(
    ...
    if snapshot.disk_available_bytes < config.min_disk_available_bytes {
        codes.push("disk_low");
        reasons.push(format!(
            "journal filesystem has {} bytes available, below {}",
...
#[cfg(target_os = "linux")]
fn probe_host(journal_path: &Path) -> Result<HostHealthSnapshot, HostHealthError> {
    let filesystem_path = journal_path.parent().unwrap_or_else(|| Path::new("."));
...
fn probe_host(_journal_path: &Path) -> Result<HostHealthSnapshot, HostHealthError> {
...
fn disk_available_bytes(path: &Path) -> Result<u64, HostHealthError> {
    ...
    let path = CString::new(path.as_os_str().as_bytes())
        .map_err(|_| HostHealthError::Probe("journal path contains a NUL byte".to_string()))?;
    ...
    if result != 0 {
        return Err(HostHealthError::Probe(format!(
            "statvfs failed for journal filesystem: {}",
...
mod tests {
    ...
    fn evaluates_every_host_threshold_fail_closed() {
        ...
        assert!(error.to_string().contains("journal filesystem"));
