# Goal G Amendment 11: Closed Review Namespace Recovery

Status: authorized for execution

Authorization date: 2026-08-03

Scope: preserve Amendment 10's terminal evidence and absent v8, authenticate
retained v5, v6, and v7, create fresh v9 solely from exact v3, carry forward
the three already reviewed bootstrap/preflight repairs, correct only the
authorization review-count namespace collision, and resume Goal G activation
under the existing one-child execution boundary

## Purpose

Amendment 10's corrected source reviews passed, exact `G10_AUTH` committed,
and its canonical bootstrap transferred control to the reviewed 17-child
launcher. The launcher authenticated repository identity, `G10_AUTH`, exact
`G9_STOP`, exact `G9_AUTH`, and first-parent lineage. Its pre-copy verifier
then rejected the authorization suffix because the unnumbered aggregate field
`goal_g_amendment_10_source_review_count=2` fell inside a namespace closed to
exactly twelve numbered per-review fields. It stopped with exit class 67 at
`source-review-evidence:exact-review-namespace`. No copy or candidate
invocation occurred, v8 remained absent, and exact `G10_STOP` recorded the
failure.

This amendment corrects only that authorization-schema collision. Its
aggregate count is `goal_g_amendment_11_review_count=2`, outside the closed
`goal_g_amendment_11_source_review_` namespace. That namespace contains
exactly the twelve numbered six-field review keys and no unnumbered key. The
new verifier proves exact namespace equality before copy.

This amendment preserves every stop, the complete Amendment 10 contract and
terminal block, absent v8, and v5/v6/v7 byte-for-byte. v5, v6, and v7 are
comparison evidence only and are never mutated, invoked, promoted, or used as
bundle inputs. One closed launcher authenticates exact `G10_STOP`, exact v3,
exact retained v5/v6/v7, absent v8, and every v9 successor absence; copies
exact v3 to fresh v9 while remaining alive; and post-verifies the copy before
releasing its success record. v9 is the only new construction root.
...
1. reconstruct the exact Amendment 9 five-target constructor repair in fresh
   v9 directly from v3;
...
This amendment retains Amendment 10's eight-field no-scratch review schema and
...
in Amendments 3 through 10. All other safety, workload, retained no-Cargo,
...
v3, retained v5, retained v6, retained v7, absent v8, and every named v9,
patch, reserved scratch, preview, official, and runtime path. No concurrent
session or process may mutate any of them. After post-copy verification, the
launcher separately rechecks exact HEAD, exact tree, and clean
tracked/untracked status before releasing buffered success.
...
```text
G10_STOP_commit=f9aa034a44205b16fd6ac8b78a47419baf5a8cc7
G10_STOP_tree=a5b6bbde99a4a268a0e340a3ebbc28c817ccee60
G10_STOP_parent=a3c85ff186dfca1b27c96d59b018848fe4a10563
G10_STOP_subject=docs: record goal g amendment 10 activation stop
G10_STOP_delta_path_count=1
G10_STOP_delta_paths=docs/polymarket-authenticated-execution-goal-g-handoff.md
G10_STOP_handoff_sha256=e1c5cf06b060a93039d494c82179f482e72406b9a8d356970cb25ca39d82d2e5
G10_STOP_handoff_bytes=187890
```

The successor chain is:

```text
R6 -> S4 -> T -> A5 -> G5_STOP -> G6_AUTH -> G6_STOP -> G7_AUTH -> G7_STOP -> G8_AUTH -> G8_STOP -> G9_AUTH -> G9_STOP -> G10_AUTH -> G10_STOP -> G11_AUTH -> G3 -> P0
```

Historical Goal G-R retains its existing aliases. This authorization is
`G11_AUTH`; no commit may be amended, replaced, skipped, or assigned another
alias.
...
Every Amendment 3, 5, 6, 7, 8, 9, and 10 terminal block is immutable.
Amendment 10 remains stopped even if Amendment 11 succeeds. Its source
reviews, authorization commit, canonical pre-copy failure, and absent-v8
evidence retain their exact historical meanings.
...
Amendment 10's reserved construction and review paths remain absent evidence:
...
target/tmp/goal-g-amendment-3-preview-v7
```

## G11_AUTH commit

`G11_AUTH` is the direct child of exact `G10_STOP`, changes only:

- `docs/polymarket-authenticated-execution-goal-g-amendment-11.md`; and
- `docs/polymarket-authenticated-execution-goal-g-handoff.md`.

Its exact subject is:

```text
docs: authorize goal g amendment 11 review namespace recovery
```

The handoff appends one `goal_g_amendment_11_status=authorized-inactive`, one
`goal_g_amendment_11_review_count=2` outside the closed source-review
namespace, and
does not modify an earlier byte. The pre-commit contract does not contain the
future G11 commit, tree, or handoff hash. The user's 2026-08-03 instruction
authorizes this amendment's execution. Its preceding push instruction applied
through exact `G10_STOP`; future Amendment 11 commits are not push-authorized.
...
```text
goal_g_amendment_11_source_review_N_result
goal_g_amendment_11_source_review_N_reviewer
goal_g_amendment_11_source_review_N_session
goal_g_amendment_11_source_review_N_contract_sha256
goal_g_amendment_11_source_review_N_bootstrap_sha256
goal_g_amendment_11_source_review_N_launcher_sha256
```
...
<!-- GOAL-G-A11-BOOTSTRAP-SOURCE-BEGIN -->
```python
import hashlib, os, sys

def die(message, code=65):
    try:
        os.write(2, ("goal-g-a11-bootstrap:" + message + "\n").encode("ascii"))
    except Exception:
        pass
...
def main():
    ...
    if contract_path != "/home/ubuntu/code/reap/docs/polymarket-authenticated-execution-goal-g-amendment-11.md":
    ...
    if len(self_bytes) != decimal_field(handoff, "goal_g_amendment_11_bootstrap_bytes"):
        die("bootstrap-bytes")
    if hashlib.sha256(self_bytes).hexdigest() != field(handoff, "goal_g_amendment_11_bootstrap_sha256"):
        die("bootstrap-sha256")
    start = b"<!-- GOAL-G-A11-LAUNCHER-SOURCE-BEGIN -->\n```bash\n"
    end = b"\n```\n<!-- GOAL-G-A11-LAUNCHER-SOURCE-END -->"
    if contract.count(start) != 1 or contract.count(end) != 1:
        die("launcher-markers")
    source = contract.split(start, 1)[1].split(end, 1)[0]
    if b"\x00" in source:
        die("launcher-nul")
    if len(source) != decimal_field(handoff, "goal_g_amendment_11_launcher_bytes"):
        die("launcher-bytes")
    if hashlib.sha256(source).hexdigest() != field(handoff, "goal_g_amendment_11_launcher_sha256"):
        die("launcher-sha256")
    try:
    ...
    os.execve(
        ...
        ["/bin/bash", "--noprofile", "--norc", "-c", decoded,
         "goal-g-a11-authenticate-and-copy"],
...
```
<!-- GOAL-G-A11-BOOTSTRAP-SOURCE-END -->

```text
bootstrap_bytes=3605
bootstrap_sha256=a33cdac88b9f8fbf09ca67307e2614ef37a0dcf910715347fff272d5c7d955f0
```

## Exact authenticate-and-copy launcher

The launcher is the complete pre-v9 authority. It executes each listed
...
<!-- GOAL-G-A11-LAUNCHER-SOURCE-BEGIN -->
```bash
set -Eeuo pipefail

readonly REPO=/home/ubuntu/code/reap
readonly CONTRACT=$REPO/docs/polymarket-authenticated-execution-goal-g-amendment-11.md
readonly HANDOFF=$REPO/docs/polymarket-authenticated-execution-goal-g-handoff.md
readonly G10_STOP=f9aa034a44205b16fd6ac8b78a47419baf5a8cc7
readonly G10_AUTH=a3c85ff186dfca1b27c96d59b018848fe4a10563
...
readonly -a CHILD_IDS=(
  repository-root repository-branch repository-clean g11-commit g11-tree
  g11-parent g11-subject g11-two-path-delta g10-stop-object g10-auth-object
  first-parent-lineage pre-copy-verifier v3-to-v9-copy post-copy-verifier
  final-g11-commit final-g11-tree final-repository-clean
)
child_index=0
begin_child() {
  local expected=$1
...
readonly -a SHELL_ASSERTION_IDS=(
  ...
  repository-clean g11-commit g11-tree g11-parent g11-subject g11-two-path-delta
  g10-stop-object g10-auth-object first-parent-lineage
...
[[ $# -eq 0 && $0 == goal-g-a11-authenticate-and-copy ]] || fail 64 invocation-schema
...
begin_shell_assertion g11-commit
capture_child g11_commit g11-commit /usr/bin/git rev-parse HEAD
[[ $g11_commit =~ ^[0-9a-f]{40}$ ]] || fail 67 g11-commit

begin_shell_assertion g11-tree
capture_child g11_tree g11-tree /usr/bin/git rev-parse 'HEAD^{tree}'
[[ $g11_tree =~ ^[0-9a-f]{40}$ ]] || fail 67 g11-tree

begin_shell_assertion g11-parent
capture_child g11_parent g11-parent /usr/bin/git rev-parse 'HEAD^'
[[ $g11_parent == "$G10_STOP" ]] || fail 67 g11-parent

begin_shell_assertion g11-subject
capture_child g11_subject g11-subject /usr/bin/git show -s --format=%s HEAD
[[ $g11_subject == 'docs: authorize goal g amendment 11 review namespace recovery' ]] || fail 67 g11-subject

begin_shell_assertion g11-two-path-delta
capture_child g11_delta g11-two-path-delta /usr/bin/git diff-tree --no-commit-id --name-only -r HEAD
[[ $g11_delta == $'docs/polymarket-authenticated-execution-goal-g-amendment-11.md\ndocs/polymarket-authenticated-execution-goal-g-handoff.md' ]] || fail 67 g11-two-path-delta

begin_shell_assertion g10-stop-object
capture_child g10_stop_object g10-stop-object /usr/bin/git show -s --format='%H%x09%T%x09%P%x09%s' "$G10_STOP"
[[ $g10_stop_object == $'f9aa034a44205b16fd6ac8b78a47419baf5a8cc7\ta5b6bbde99a4a268a0e340a3ebbc28c817ccee60\ta3c85ff186dfca1b27c96d59b018848fe4a10563\tdocs: record goal g amendment 10 activation stop' ]] || fail 67 g10-stop-object

begin_shell_assertion g10-auth-object
capture_child g10_auth_object g10-auth-object /usr/bin/git show -s --format='%H%x09%T%x09%P%x09%s' "$G10_AUTH"
[[ $g10_auth_object == $'a3c85ff186dfca1b27c96d59b018848fe4a10563\t682e01042034f48986797117daf0b4ed9b75ff76\t6e04e60fa8f2412c87d51d28136cc23546ad8805\tdocs: authorize goal g amendment 10 complete entrypoint bootstrap recovery' ]] || fail 67 g10-auth-object

begin_shell_assertion first-parent-lineage
capture_child lineage first-parent-lineage /usr/bin/git rev-list --first-parent --max-count=16 HEAD
...
export GOAL_G_A11_REPO=$REPO
export GOAL_G_A11_CONTRACT=$CONTRACT
export GOAL_G_A11_HANDOFF=$HANDOFF
export GOAL_G_A11_G11_COMMIT=$g11_commit
export GOAL_G_A11_G11_TREE=$g11_tree
export GOAL_G_A11_G11_PARENT=$g11_parent
export GOAL_G_A11_G11_SUBJECT=$g11_subject
...
def abort(code, assertion, detail):
    try:
        os.write(2, ("goal-g-a11:failure:%d:%s:%s\n" %
...
try:
    repo = os.environ["GOAL_G_A11_REPO"]
    contract_path = os.environ["GOAL_G_A11_CONTRACT"]
    handoff_path = os.environ["GOAL_G_A11_HANDOFF"]
    g11_commit = os.environ["GOAL_G_A11_G11_COMMIT"]
    g11_tree = os.environ["GOAL_G_A11_G11_TREE"]
    g11_parent = os.environ["GOAL_G_A11_G11_PARENT"]
    g11_subject = os.environ["GOAL_G_A11_G11_SUBJECT"]
    ...
    equal(hashlib.sha256(handoff_data[:parent_handoff_bytes]).hexdigest(),
          "e1c5cf06b060a93039d494c82179f482e72406b9a8d356970cb25ca39d82d2e5",
          67, "handoff-status-and-cross-binding", "parent-prefix")
    suffix = handoff_data[parent_handoff_bytes:]
    suffix_start = b"\n## User-Authorized Amendment 11 \xe2\x80\x94 2026-08-03\n"
    ...
    for name, expected in (
        ...
        ("goal_g_amendment_10_status", "activation-stopped-inactive"),
        ("goal_g_amendment_11_status", "authorized-inactive"),
        ("goal_g_amendment_11_contract_sha256", contract_hash),
    ...
    equal((g11_parent, g11_subject),
          ("f9aa034a44205b16fd6ac8b78a47419baf5a8cc7",
           "docs: authorize goal g amendment 11 review namespace recovery"),
          67, "handoff-status-and-cross-binding", "g11-runtime")

    begin("source-review-evidence")
    equal(unique(values, "goal_g_amendment_11_review_count",
                 "source-review-evidence"), "2", 67,
          "source-review-evidence", "review-count")
    bootstrap_hash = unique(values, "goal_g_amendment_11_bootstrap_sha256", "source-review-evidence")
    launcher_hash = unique(values, "goal_g_amendment_11_launcher_sha256", "source-review-evidence")
    allowed_review_keys = set()
    for number, reviewer, session in (
        ("1", "g11-contract-review-1", "g11-contract-review-1-20260803"),
        ("2", "g11-boundary-review-2", "g11-boundary-review-2-20260803"),
    ):
        prefix = "goal_g_amendment_11_source_review_" + number + "_"
        expected = {
    ...
    for name, expected in (
        ...
        equal(unique(values, name, "a10-terminal-evidence"), expected,
              67, "a10-terminal-evidence", name)
    ...
    for path in (
        ...
        "/var/tmp/reap-g3-draft-v9",
        "/var/tmp/reap-g3-draft-v9-provenance.patch",
        "/var/tmp/reap-g3-draft-v9-review-1-scratch",
        "/var/tmp/reap-g3-draft-v9-review-2-scratch",
        repo + "/target/tmp/goal-g-amendment-3-preview-v8",
    ...
    argv = ("/bin/busybox", "cp", "-a", "--",
            "/var/tmp/reap-g3-draft-v3", "/var/tmp/reap-g3-draft-v9")
    stream = b"".join(value.encode() + b"\0" for value in argv)
    equal((len(argv), len(stream), hashlib.sha256(stream).hexdigest()),
          (6, 74, "08d79b63a6b748be66c41fc813f3b59217139f8b4bff201031461a1b2788e5c3"),
    ...
    for name, value in (
        ("goal_g_a11_pre_contract_sha256", contract_hash),
        ("goal_g_a11_pre_handoff_sha256", handoff_hash),
    ):
...
pre_contract_sha256=${pre_contract_line#goal_g_a11_pre_contract_sha256=}
pre_handoff_sha256=${pre_handoff_line#goal_g_a11_pre_handoff_sha256=}
[[ $pre_contract_line == goal_g_a11_pre_contract_sha256="$pre_contract_sha256" && $pre_contract_sha256 =~ ^[0-9a-f]{64}$ ]] || fail 67 verifier-contract-output
[[ $pre_handoff_line == goal_g_a11_pre_handoff_sha256="$pre_handoff_sha256" && $pre_handoff_sha256 =~ ^[0-9a-f]{64}$ ]] || fail 67 verifier-handoff-output
readonly pre_contract_sha256 pre_handoff_sha256
export GOAL_G_A11_PRE_CONTRACT_SHA256=$pre_contract_sha256
export GOAL_G_A11_PRE_HANDOFF_SHA256=$pre_handoff_sha256

begin_child v3-to-v9-copy
set +e
(
  storage_preflight || exit 125
  exec /bin/busybox cp -a -- /var/tmp/reap-g3-draft-v3 /var/tmp/reap-g3-draft-v9
)
copy_status=$?
set -e
...
def abort(code, detail):
    try:
        os.write(2, ("goal-g-a11:post-copy-failure:%d:%s\n" %
...
try:
    ...
    v9 = direct_tree("/var/tmp/reap-g3-draft-v9", None, True)
    if v9 in (v3, v5, v6, v7):
        abort(68, "fresh-root-alias")
    for path in (
        "/var/tmp/reap-g3-draft-v6-provenance.patch",
        "/var/tmp/reap-g3-draft-v6-review-1-scratch",
        "/var/tmp/reap-g3-draft-v6-review-2-scratch",
        os.environ["GOAL_G_A11_REPO"] + "/target/tmp/goal-g-amendment-3-preview-v5",
        "/var/tmp/reap-g3-draft-v7-provenance.patch",
        "/var/tmp/reap-g3-draft-v7-review-1-scratch",
        "/var/tmp/reap-g3-draft-v7-review-2-scratch",
        os.environ["GOAL_G_A11_REPO"] + "/target/tmp/goal-g-amendment-3-preview-v6",
        "/var/tmp/reap-g3-draft-v8",
        "/var/tmp/reap-g3-draft-v8-provenance.patch",
        "/var/tmp/reap-g3-draft-v8-review-1-scratch",
        "/var/tmp/reap-g3-draft-v8-review-2-scratch",
        os.environ["GOAL_G_A11_REPO"] + "/target/tmp/goal-g-amendment-3-preview-v7",
        "/var/tmp/reap-g3-draft-v9-provenance.patch",
        "/var/tmp/reap-g3-draft-v9-review-1-scratch",
        "/var/tmp/reap-g3-draft-v9-review-2-scratch",
        os.environ["GOAL_G_A11_REPO"] + "/target/tmp/goal-g-amendment-3-preview-v8",
        os.environ["GOAL_G_A11_REPO"] + "/target/tmp/goal-g-amendment-3-recorder-bundle",
        os.environ["GOAL_G_A11_REPO"] + "/target/tmp/goal-g-phase0-amendment-3",
        os.environ["GOAL_G_A11_REPO"] + "/target/tmp/goal-g-amendment-3-runtime",
    ):
        if os.path.lexists(path):
            abort(71, "post-copy-auxiliary-present")
    contract_hash, _ = stable_document(os.environ["GOAL_G_A11_CONTRACT"])
    handoff_hash, _ = stable_document(os.environ["GOAL_G_A11_HANDOFF"])
    equal(contract_hash, os.environ["GOAL_G_A11_PRE_CONTRACT_SHA256"],
          72, "contract-pre-post-hash")
    equal(handoff_hash, os.environ["GOAL_G_A11_PRE_HANDOFF_SHA256"],
          72, "handoff-pre-post-hash")
    for name, value in (
        ("goal_g_amendment_11_result", "authenticate-copy-postverify-pass"),
        ("goal_g_amendment_11_g11_auth_commit", os.environ["GOAL_G_A11_G11_COMMIT"]),
        ("goal_g_amendment_11_g11_auth_tree", os.environ["GOAL_G_A11_G11_TREE"]),
        ("goal_g_amendment_11_g11_auth_parent", os.environ["GOAL_G_A11_G11_PARENT"]),
        ("goal_g_amendment_11_g11_auth_subject", os.environ["GOAL_G_A11_G11_SUBJECT"]),
        ("goal_g_amendment_11_g11_auth_contract_sha256", contract_hash),
        ("goal_g_amendment_11_g11_auth_handoff_sha256", handoff_hash),
        ("goal_g_amendment_11_v9_root_dev", str(v9[0])),
        ("goal_g_amendment_11_v9_root_inode", str(v9[1])),
        ("goal_g_amendment_11_v9_forensic_inventory_sha256",
...
capture_child final_g11_commit final-g11-commit /usr/bin/git rev-parse HEAD
[[ $final_g11_commit == "$g11_commit" ]] || fail 67 final-g11-commit

capture_child final_g11_tree final-g11-tree /usr/bin/git rev-parse 'HEAD^{tree}'
[[ $final_g11_tree == "$g11_tree" ]] || fail 67 final-g11-tree
...
```text
launcher_bytes=36248
launcher_sha256=64b9034be7b1c83cb4e251a97a868d31f45aef75c0325c896561176b2953a762
...
argv_count=6
argv_nul_bytes=74
argv_nul_sha256=08d79b63a6b748be66c41fc813f3b59217139f8b4bff201031461a1b2788e5c3
```
...
```text
v9_root=/var/tmp/reap-g3-draft-v9
v9_patch=/var/tmp/reap-g3-draft-v9-provenance.patch
review_1_scratch=/var/tmp/reap-g3-draft-v9-review-1-scratch
review_2_scratch=/var/tmp/reap-g3-draft-v9-review-2-scratch
preview_root=target/tmp/goal-g-amendment-3-preview-v8
```

v3 is the sole source/control. v5, v6, and v7 remain immutable evidence, and
all v8 paths remain immutable absent Amendment 10 evidence.
Exactly these six v9 files may differ from v3:
...
body-hash anchors remain controlling with `v4` read as `v9`, except for the
...
fields and add exactly these 66 fields:
...
g10_stop_commit g10_stop_tree g10_stop_parent g10_stop_subject g10_stop_handoff_sha256
g11_auth_commit g11_auth_tree g11_auth_parent g11_auth_subject g11_auth_contract_sha256 g11_auth_handoff_sha256
```

`candidate_parent` is exact `G11_AUTH`. No `a6_*` synonym is authorized.
...
six-section Git full-index text patch directly from v3 to v9.

### Exact no-scratch static-review schema

Only for v9 static reviews, this amendment supersedes Amendment 6's eight-
...
```text
goal_g_amendment_11_v9_review_N_result
goal_g_amendment_11_v9_review_N_reviewer
goal_g_amendment_11_v9_review_N_session
goal_g_amendment_11_v9_review_N_implementation_sha256
goal_g_amendment_11_v9_review_N_scratch_state
goal_g_amendment_11_v9_review_N_v3_forensic_inventory_sha256
goal_g_amendment_11_v9_review_N_v9_forensic_inventory_sha256
goal_g_amendment_11_v9_review_N_patch_sha256
```
...
`absent-not-created`; both v3 hashes, both v9 hashes, and both patch hashes
agree with the frozen inputs. Any `scratch_final_inventory_sha256` field,
Amendment 6/8 synonym, missing, duplicate, malformed, or extra Amendment 11
v9-review field is forbidden. `verify_bundle`, activation validation,
...
memory, it independently reproduces the complete direct v3-to-v9 patch,
...
/bin/busybox
sh
/var/tmp/reap-g3-draft-v9/construct-self-test.preview.sh
preview
/home/ubuntu/code/reap/target/tmp/goal-g-amendment-3-preview-v8
argv_count=5
argv_nul_bytes=145
argv_nul_sha256=806e26f5cd8987fb7a9eb8ac355c2f7978a6333f876b54402049e780d5bca76f
```

The v9 constructor reconstructs the exact A9-reviewed bootstrap directly from
...
argv_count=4
argv_nul_bytes=92
argv_nul_sha256=0083ab973c7160c2e6eb5e5b678c76e9623d134ec13787f23336230bd595524f
```
...
If every gate passes, `G3` is the direct child of exact `G11_AUTH`, changes
...
goal_g_amendment_11_status: authorized-inactive -> activation-complete-phase0-active
```

Amendments 5, 6, 7, 8, 9, and 10 remain stopped. `P0` remains the direct child of
...
When storage permits, a stop commit is the direct child of `G11_AUTH`, changes
only the handoff, replaces the one Amendment 11 status with
`activation-stopped-inactive`, appends one terminal block, and uses subject:

```text
docs: record goal g amendment 11 activation stop
```
...
v8_creation_reuse_or_promotion_authorized=false
