...
| `OkxNitroSubscriberBase` 15-second application ping, `SharedSessionSubscriberBase` session ownership/fixed reconnect interval, `WsConnectOption` three-miss default, and low-level INIT/ping/pong/data liveness checks | Shutdown/recovery-cancellable 10-second feed handshake, 5-second bounded control writes, 15-second application ping, 30-second no-frame threshold, retained per-socket recovery ownership, readiness-reset exponential reconnect supervision, one-second private-output backpressure bound, book/reference freshness, and lossless bounded ready/disconnect delivery | Current-contract hardening: Rust backs off repeated pre-readiness failures but clears historical delay after exact subscription readiness. It prevents an unbounded handshake, write, private event-queue wait, or closed recovery receiver from stalling/spinning the supervisor. Private saturation revokes readiness and drives cancellation/reconciliation rather than silently dropping account/order data or hanging. Transport frames establish socket liveness; independently aged strategy and account components prevent pongs from masking stale required data. Low-frequency funding channels are not assigned an invalid global payload cadence. |
...
| `OkxNitroExchStatusClient` 10-second `/api/v5/system/status` polling plus `ExchStatusSafeguard` 60-second lead and `OkxNitroUtils.getExchStatus` service filter | Typed bootstrap and periodic checks derive unified websocket/trading/account/product/ambiguous relevance and configured regular products from the resolved Chaos plan, filter `env`, validate endpoint rate, and exclude spread-only maintenance | Current-contract hardening: Java pauses and may resume the strategy; Rust enters fail-closed cancel/reconcile shutdown and requires a clean restart, while ambiguous possibly relevant maintenance still fails closed |
