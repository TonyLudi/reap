...
| --- | --- | --- |
| `Iarb2Strategy` market/fill loop | `src/strategy/chaos.rs` | Depth updates refresh risk, hedges, theo quotes, and quote orders. Quote fills trigger IOC hedges. |
...
| `QueueMatchingEngine` | `src/backtest/matching.rs` | Supports `PostOnly`, `IOC`, current-depth taker fills, later maker fills from trades/depth, and queue-ahead tracking. |
