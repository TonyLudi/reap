...
| `reap-pm-live` | Sibling PM public capture/replay, read-only monitor, deterministic lanes, one-owner coordinator, Goal F/authenticated journals, fake and authenticated-loopback roots, plus the strategy-neutral production supervisor with leased recovery, a concrete authenticated private-WS order/trade mapper, complete condition-scoped order/trade and per-token position polling, fixed heartbeat and dual-token mutation adapters, fill-derived position reconciliation, and zero-order shutdown. |
